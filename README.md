...
* 支持筛选输入自动补全、表达式历史和常用筛选保存
* 支持单列排序、Shift + 点击多列组合排序
* 支持拖拽列头右侧手柄调整列宽
* 支持列头右键菜单，可复制变量名/标签、排序、隐藏列、重置列宽和打开变量统计
* 支持数据单元格右键菜单，可复制当前单元格或将当前行复制为变量=值文本
* 显示变量标签与值标签，并可在原始值、标签、原始值 + 标签之间切换显示方式
* 可通过侧边栏隐藏 / 显示指定列，并可高亮显示缺失值
* 内置变量字典：集中查看变量名、变量标签、存储类型、统计类型、有效数、缺失率和唯一值数
* 内置变量探索器：支持单变量频次分析、描述性统计
* 点击表格行可打开行详情，以转置表查看当前行的变量、标签、值和值标签
* 变量统计会以可视标记展示纯空白字符串，便于区分缺失值和空白字符串
* 支持将当前表格视图或变量字典导出为 CSV 或 Excel
* 内置文件信息弹窗，可查看路径、大小、更新时间、Stata 版本、字节序、数据量和变量数
* 内置使用说明弹窗，快速查看筛选表达式和表格操作说明
* 可识别部分更早的 Stata .dta 格式，并提示清晰的不支持版本信息
* 底层文件更新时，自动刷新视图
...
* 点击 .dta 文件即可直接打开
* 通过顶部工具栏输入条件筛选行数据
* 筛选框支持变量名/函数自动补全，可从历史和常用筛选中复用表达式
* 点击列标题实现排序；按住 Shift 点击多个列标题可进行多列排序
* 拖拽列标题右侧手柄可调整列宽
* 右键点击列标题可打开列操作菜单
* 右键点击数据单元格可复制当前单元格，或将当前行复制为变量=值的多行文本
* 可在值标签显示控件中切换原始值、标签、原始值 + 标签
* 点击数据行可打开行详情；再次点击已选中的行可关闭详情
* 行详情面板支持拖拽调整高度，详情表列宽也可拖拽调整
* 侧边栏支持搜索变量、隐藏 / 显示指定列，并可切换缺失值高亮显示
* 点击变量名可向筛选框插入变量名或函数模板
* 可打开使用说明、变量字典和文件信息，也可刷新或导出数据
* 打开文件信息、使用说明、变量字典或变量统计弹窗后，可按 Esc 关闭
...
变量字典弹窗也支持导出为 CSV 或 Excel，便于生成文档或 codebook。

### 变量字典

变量字典用于集中查看变量结构、标签、存储类型、统计类型、有效数、缺失率和唯一值数。
...
* 纯空白字符串会显示为 `␠×2` 等可视标记，不会与真正的缺失值混淆
